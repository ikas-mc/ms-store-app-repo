(function () {
  ...
  function convertCategory(value) {
    ...
    for (let i = 0; i < categoryMapper.length; i++) {
      let mapper = categoryMapper[i];
      if (mapper.regex.test(value)) {
        return mapper.value;
      }
    }
  ...
  chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    repo.productId = location.pathname.substring(location.pathname.lastIndexOf("/") + 1);
    repo.url = "https://www.microsoft.com/store/productId/" + repo.productId;
    download(repo.name.replace(/\s+/g, "-") + ".json", JSON.stringify(repo, null, 4));
    sendResponse("end");
  });
